...
describe('validate', () => {
  ...
  describe('#user', () => {
    ...
    it('show throw if password does not match', () => {
      expect(() => validate.user({ password: 'password' }, 'otherpassword')).to.throw('User password does not match');
    });

    it('show return user if password matches', () => {
      expect(validate.user({ password: 'password' }, 'password')).to.eql({ password: 'password' });
    });

    it('show return user if password matches with data', () => {
      expect(validate.user({ user: 'yo', password: 'password' }, 'password')).to.eql({ user: 'yo', password: 'password' });
    });
  });
  ...
  describe('#userExists', () => {
    ...
    it('show return user if it exists', () => {
      expect(validate.userExists({ password: 'password' })).to.eql({ password: 'password' });
    });
  });
  ...
  describe('#client', () => {
    ...
    it('show throw if client secret does not match', () => {
      expect(() => validate.client({ clientSecret: 'password' }, 'otherpassword')).to.throw('Client secret does not match');
    });

    it('show return client if client secret matches', () => {
      expect(validate.client({ clientSecret: 'password' }, 'password')).to.eql({ clientSecret: 'password' });
    });

    it('show return client if password matches with data', () => {
      expect(validate.client({ client: 'yo', clientSecret: 'password' }, 'password')).to.eql({ client: 'yo', clientSecret: 'password' });
    });
  });
  ...
  describe('#clientExists', () => {
    ...
    it('show return user if it exists', () => {
      expect(validate.clientExists({ clientSecret: 'password' })).to.eql({ clientSecret: 'password' });
    });
  });

  describe('#token', () => {
    it('should throw with undefined code', () => {
      expect(() => validate.token({ userID: '1' }, undefined)).to.throw('JsonWebTokenError: jwt must be provided');
    });

    it('should throw with null code', () => {
      expect(() => validate.token({ userID: '1' }, null)).to.throw('JsonWebTokenError: jwt must be provided');
    });

    it('should throw with invalid userID', () => {
      const token = utils.createToken();
      return validate.token({ userID: '-1' }, token).catch(err => expect(err.message).to.eql('User does not exist'));
    });

    it('should throw with invalid clientID', () => {
      const token = utils.createToken();
      return validate.token({ clientID: '-1' }, token).catch(err => expect(err.message).to.eql('Client does not exist'));
    });

    it('should throw with invalid userID and invalid clientID', () => {
      const token = utils.createToken();
      return validate.token({ userID: '-1', clientID: '-1' }, token).catch(err => expect(err.message).to.eql('User does not exist'));
    });

    it('should return user with valid user', () => {
      const token = utils.createToken();
      const user = { userID: '1' };
      return validate.token(user, token).then(returnedUser => expect(returnedUser.id).eql(user.userID));
    });

    it('should return client with valid client', () => {
      const token = utils.createToken();
      const client = { clientID: '1' };
      return validate.token(client, token).then(returnedClient => expect(returnedClient.id).eql(client.clientID));
    });
  });
  ...
  describe('#generateRefreshToken', () => {
    it('should generate and return a refresh token', () => validate.generateRefreshToken({ userID: '1', clientID: '1', scope: '*' }).then(token => utils.verifyToken(token)));
  });

  describe('#generateToken', () => {
    it('should generate and return a token', () => validate.generateToken({ userID: '1', clientID: '1', scope: '*' }).then(token => utils.verifyToken(token)));
  });

  describe('#generateTokens', () => {
    it('should generate and return an access and refresh token', () => validate.generateTokens({ userID: '1', clientID: '1', scope: 'offline_access' }).then(([accessToken, refreshToken]) => {
      utils.verifyToken(accessToken);
      utils.verifyToken(refreshToken);
    }));

    it('should generate and return an access with no refresh token when scope is defined as all', () => validate.generateTokens({ userID: '1', clientID: '1', scope: '*' }).then(([accessToken, refreshToken]) => {
      utils.verifyToken(accessToken);
  ...
  describe('#tokenForHttp', () => {
    ...
    it('should work with a valid token', () => {
      const token = utils.createToken();
      return validate.tokenForHttp(token).then(returnedToken => expect(returnedToken).to.eql(token));
    });
  });
